extends Node2D

class_name Map

class WaveData:
	var start: float
	var spaws: int
	var cooldown: float

var el_path: Path2D
@onready var el_coins: Label = get_node("%CoinLabel")
@onready var el_cheese: Label = get_node("%CheeseLabel")
@onready var el_pause: PauseMenu = get_node("%PauseMenu")
@onready var el_game_over: PauseMenu = get_node("%GameOverMenu")
@onready var el_win: PauseMenu = get_node("%WinMenu")
@onready var el_time: Label = get_node("%TimeLabel")
@onready var el_lifebar: ProgressBar = get_node("%LifeBar")
@onready var el_mousebar: ProgressBar = get_node("%MouseBar")

var mouse_scene: PackedScene = preload("res://scenes/Mouse.tscn")
var data: Dictionary = GameData.cat_data
@onready var action_buttons: Array[Node] = get_tree().get_nodes_in_group("action_button")
var tooltips: Array[Tooltip] = []
var all_buttons: Array[CircleButton] = []
# the cat currently being built and dragged by the mouse
var cat_building: Cat = null
var cat_selected: Cat = null
@export var next_map: String = "Map02"
@export var coins: float = 20.0
@export var life: float = 20.0
@onready var max_life: float = life
@export var max_cell_x: int = 10
@export var max_cell_y: int = 5
@export var size_curve: Curve
#export var final_time := 500.0
#export var final_size := 1600.0
@export var wave_list := [
	{
		"start": 1,
		"spawns": 10,
		"cooldown": 4.0,
		"size_min": 50,
		"size_max": 80
	},
	{
		"start": 16,
		"spawns": 8,
		"cooldown": 0.08,
		"size_min": 2,
		"size_max": 2
	},
	{
		"start": 26,
		"spawns": 12,
		"cooldown": 0.05,
		"size_min": 2,
		"size_max": 2
	},
	{
		"start": 38,
		"spawns": 4,
		"cooldown": 5.0,
		"size_min": 180,
		"size_max": 240
	},
	{
		"start": 35,
		"spawns": 18,
		"cooldown": 0.1,
		"size_min": 2,
		"size_max": 2
	}
]
var wave_state_list := [
	{
		"spawned": 0,
		"timer": null
	}
]
var cats_dict: Dictionary = {
#	[0, 0]: "cat1",
#	[1, 2]: "cat2"
}
var spawn_max := 0
var total_worth: float = 0.0
var spawn_count := 0
var kill_count := 0
var kill_worth := 0.0
var stolen_count := 0
var stolen_worth := 0.0
var start_time := 0
var ellapsed := 0
var max_size := 30.0
var paused: bool = false
enum State {READY, OVER}
var state: int = State.READY

# Called when the node enters the scene tree for the first time.
func _ready() -> void:
#	if Global.DEBUG_WIN:
#		Engine.time_scale = 4
	Engine.time_scale = 1
	start_time = Time.get_ticks_msec()
	el_path = $Path2D
	for b: Node in action_buttons:
		var button: CircleButton = b as CircleButton
		var cat_data: Dictionary = data[button.name]
		var cost: int = cat_data.cost
		var cat_name: String = cat_data.full_name
		var desc: String = tr(cat_data.description)
		b.el_tooltip.set_label(cat_name, desc)
		b.connect("pressed", Callable(self, "action_pressed").bind(b.get_name(), b))
		b.connect("button_up", Callable(self, "action_released").bind(b.get_name(), b))
		button.update_cost(cost)
		button.register_tooltip()
	ini_waves()
	update_coins()
	update_UI_mousebar()
	update_life()
	pause_game(false)
	play_music()

func _physics_process(_delta: float) -> void:
	if cat_building != null:
		cat_building.position = snap_to_grid(get_global_mouse_position())
		if can_build(get_global_mouse_position()):
			cat_building.modulate = Color.YELLOW#("ff55ff99")
		else:
			cat_building.modulate = Color.RED#("bbaa2222")

func ini_waves() -> void:
#	print('ini waves')
	spawn_max = 0
	var i: int = 0
	for wave: Dictionary in wave_list:
		spawn_max += wave.spawns
		total_worth += wave.spawns * Mouse.calculate_worth(wave.size_max)
		build_wave(wave, i)
		i += 1

func build_wave(wave: Dictionary, i: int) -> void:
#	print('building wave %s' % wave)

	var t: Timer = Timer.new()
	add_child(t)
	var wave_state: Dictionary = {
		"spawned": 0,
		"timer": t
	}
	if wave_state_list.size() > i:
		wave_state_list[i] = wave_state
	else:
		wave_state_list.append(wave_state)
	t.connect("timeout", Callable(self, "wave_timer_timeout").bind(i))
	await get_tree().create_timer(wave.start, false).timeout
	print('starting wave %s' % i)
	wave_timer_timeout(i)
	t.start(wave.cooldown)

func wave_timer_timeout(i: int) -> void:
	var wave: Dictionary = wave_list[i]
	var wave_state: Dictionary = wave_state_list[i]
	spawn_new_mouse(wave.size_min, wave.size_max)
	wave_state.spawned += 1
	var spawns_left: int = wave.spawns - wave_state.spawned
	if spawns_left == 0:
		var t: Timer = wave_state.timer
		t.stop()
		t.queue_free()
	if Global.DEBUG_WAVES:
		print('%s: wave %s spawned %s/%s' % [ellapsed, i, wave_state.spawned, wave.spawns])

func play_music() -> void:
	await get_tree().create_timer(0.4).timeout
	$AudioMusic.play()

func register_new_button(button: CircleButton) -> void:
	all_buttons.append(button)
	var _e: int = 0
	_e = button.connect("mouse_entered", Callable(self, "button_entered").bind(button))
	_e = button.connect("mouse_exited", Callable(self, "button_exited").bind(button))
	_e = button.connect("button_up", Callable(self, "hide_tooltip_on").bind(button))

# adds tooltip to array of tooltips so I can later hide all of them when one is shown
# changes tooltip's parent from the button and places it on the tooltip layer
func register_new_tooltip(tip: Tooltip) -> void:
	tooltips.append(tip)
	var pos: Vector2 = tip.get_global_transform().origin
	tip.get_parent().remove_child(tip)
	$UI/Tooltips.add_child(tip)
	tip.set_global_position(pos)
	tip.registered = true

func button_entered(button: CircleButton) -> void:
	if Config.confirm and button.confirm: return
	show_tooltip_on(button)

func button_exited(button: CircleButton) -> void:
	button.hide_tooltip()

# can be either a CircleButton or a Cat
func show_tooltip_on(button: Variant, duration: float = -1.0) -> void:
	for t: Tooltip in tooltips:
		t.hide()
	button.show_tooltip(duration)

# can eb either a CircleButton or a Cat
func hide_tooltip_on(button: Variant) -> void:
	button.hide_tooltip()

func action_pressed(cat_key: String, _button: CircleButton) -> void:
	if cat_building:
		return
	if coins < GameData.cat_data[cat_key]['cost']:
		return
	cat_building = GameData.get_cat_scene(cat_key).instantiate() as Cat
	cat_building.init(self)
	$UI.add_child(cat_building)
#	if !DisplayServer.has_feature(DisplayServer.FEATURE_TOUCHSCREEN):
#		print('no touch')
#		return
#	show_tooltip_on(button)


func action_released(cat_key: String, _button: CircleButton) -> void:
	if cat_building:
		if can_build(cat_building.global_position):
			build_cat(cat_key)
		else:
			cancel_build()
		
func build_cat(name: String) -> void:
	cat_building.position = snap_to_grid(cat_building.global_position)
	var cell_pos: Vector2i = $TileMap.local_to_map(cat_building.global_position)
	if cats_dict.get(cell_pos):
		print("already a cat at %s is %s" % [cell_pos, cats_dict.get(cell_pos)])
		cancel_build()
		return
	else:
#		print("building cat at %s" % cell_pos)
		cats_dict[cell_pos] = cat_building
	$UI.remove_child(cat_building)
	$Actors.add_child(cat_building)
	
	cat_building.connect("cat_select", Callable(self, "select_cat").bind(cat_building))
	cat_building.connect("cat_deselect", Callable(self, "unselect_cat").bind(cat_building))
	cat_building.connect("clicked", Callable(self, "_on_cat_clicked").bind(cat_building))
	cat_building.connect("shot", Callable(self, "_on_cat_shoot"))
	cat_building.done_building(cell_pos)
	cat_building = null
	add_coins(-data[name]['cost'])
	
func cancel_build() -> void:
	cat_building.queue_free()
	cat_building = null
	
func remove_cat_at_cell(cell_pos: Vector2i) -> void:
	cats_dict[cell_pos] = null
	
#func is_paused() -> bool:
#	return get_tree().paused

func toggle_pause() -> void:
	if state != State.READY: return
	el_pause.toogle()
#	pause_game(!paused)

func pause_game(new_paused: bool) -> void:
	paused = new_paused
	el_pause.visible = paused
#	print("paused: %s" % paused)
#	get_tree().paused = new_paused
	el_pause.pause(paused)

func update_UI_time() -> void:
	el_time.text = str(ellapsed)#.pad_zeros(3)
#	el_mousebar.value = clamp(ellapsed, 0, 100)

func update_UI_mousebar() -> void:
	# kills needed to win, after killing this many, you can't lose anymore
#	var kills_needed = spawn_max - max_life + 1
	var kills_needed: float = total_worth - max_life + 1.0
	# how many mice left to kill to get to the kills needed to win
	var worth_left: float = max(kills_needed - kill_worth, 0.0)
	var value: float = 0.0
	if kills_needed > 0.0:
		value = ceil(100.0 * worth_left / kills_needed)
	el_mousebar.value = clamp(value, 0.0, 100.0)
#	if mouse_left == 0:
#		print('cannot lose anymore')
	var stolen: float = max_life - life
#	print("kill worth %s + stolen %s >= total worth %s (%s) and life %s, worth left: %s, worth needed: %s, value: %s" % [kill_worth, stolen, total_worth, (kill_worth + stolen >= total_worth),life, worth_left, kills_needed, value])
	# round() is to workaround float precision
	if round(kill_worth) + round(stolen) >= round(total_worth) and life > 0:
		win()

func win() -> void:
	Engine.time_scale = 1
	print('you win')
	await get_tree().create_timer(2).timeout
	if life == max_life:
		print("Puurfect!")
	state = State.OVER
	el_win.slow_pause()
	
func add_life(ammount: float) -> void:
	life += ammount
	life = max(0, life)
	update_life()
	
func update_life() -> void:
	el_cheese.text = "%s" % int(life)
	el_lifebar.value = life * 100 / max_life
	if life == 0:
		game_over()
	
func game_over() -> void:
	state = State.OVER
	Engine.time_scale = 1
	el_game_over.slow_pause()

func add_coins(ammount: float) -> void:
	coins += ammount
	update_coins()

func update_coins() -> void:
	el_coins.text = "$%s" % int(floor(coins))
	for b: Node in action_buttons:
		var button: CircleButton = b as CircleButton
		button.set_state_from_coins(coins)
	if is_instance_valid(cat_selected):
		cat_selected.on_map_coins_changed(coins)
	
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("pause"):
		toggle_pause()
	if event.is_pressed():
#		print('new event %s at %s' % [event, get_global_mouse_position()])
		if get_cat_at(get_global_mouse_position()) == cat_selected:
			return
		if is_instance_valid(cat_selected):
			unselect_cat(cat_selected)

func snap_to_grid(position: Vector2) -> Vector2:
	# NOTE: Godot 4 map_to_local already returns the cell center
	# (Godot 3 map_to_world returned the corner, hence the old +64).
	var map_position: Vector2i = $TileMap.local_to_map(position)
	return $TileMap.map_to_local(map_position)

func can_build(position: Vector2) -> bool:
	var cell_pos: Vector2i = $TileMap.local_to_map(position)
	var id: int = $TileRoad.get_cell_source_id(0, cell_pos)
	
	if id == 0 : return false
	if cats_dict.get(cell_pos): return false
	if !is_inside_map(cell_pos): return false
	return true

# returns the cat located at given global mouse position
func get_cat_at(position: Vector2) -> Cat:
	var cell_pos: Vector2i = $TileMap.local_to_map(position)
	return cats_dict.get(cell_pos) as Cat

func is_inside_map(cell_pos: Vector2i) -> bool:
	var x1: int = 0
	var y1: int = 0
	var x2: int = max_cell_x
	var y2: int = max_cell_y
	if cell_pos.x < x1 or cell_pos.x > x2: return false
	if cell_pos.y < y1 or cell_pos.y > y2: return false
	return true

func spawn_new_mouse(s1: float = 30.0, s2: float = 60.0) -> void:
	if (spawn_count >= spawn_max): 
		$SpawnTimer.stop()
		return
	spawn_count += 1
	var mouse: Mouse = mouse_scene.instantiate()
	mouse.position = Vector2(0, 0)
	
	var min_size: float = s1
#	max_size = min_size + pow(ellapsed, 2.0) * 0.0100
#	var interp = size_curve.interpolate(ellapsed / final_time)
#	max_size = min_size + interp * (final_size - min_size)
#	print('ellpased: %s, interp: %s, maxsize: %s' % [ellapsed, interp, max_size])
	if Global.DEBUG_WIN:
		max_size = 40
	max_size = s2
	var h: float = randf_range(min_size, max_size)
	# makes it exponential distribution, so big sizes are more rare, 
	# while keeping same min and max sizes
	h = min_size + pow(h, 4) * (min_size + max_size) / pow(max_size + min_size, 4)
	mouse.max_health = h
	
	var min_speed: int = 35
	var max_speed: float = min(110.0, min_speed + ellapsed * 0.155)
	var s: float = randf_range(min_speed, max_speed)
	s = min_speed + pow(s, 3) * (min_speed + max_speed) / pow(min_speed + max_speed, 3)
	mouse.speed = s
	mouse.update_worth(s2)
	el_path.add_child(mouse)
# warning-ignore:return_value_discarded
	mouse.connect("killed", Callable(self, "_on_mouse_killed").bind(mouse))
# warning-ignore:return_value_discarded
	mouse.connect("cheese", Callable(self, "_on_mouse_reached_cheese").bind(mouse))

func _on_SpawnTimer_timeout() -> void:
	spawn_new_mouse()
	if $SpawnTimer.wait_time >= 3.0:
		$SpawnTimer.wait_time -= 0.2#$SpawnTimer.wait_time * 0.05
	elif $SpawnTimer.wait_time >= 2.0:
		$SpawnTimer.wait_time -= $SpawnTimer.wait_time * 0.01
	elif $SpawnTimer.wait_time >= 1.2:
		$SpawnTimer.wait_time -= $SpawnTimer.wait_time * 0.005
	elif $SpawnTimer.wait_time >= 0.5:
		$SpawnTimer.wait_time -= $SpawnTimer.wait_time * 0.002
	if $SpawnTimer.wait_time >= 0.5 and Global.DEBUG_MAP_TIMER:
		print("new spawn timer is %f" % $SpawnTimer.wait_time)
	
	
func _on_mouse_killed(mouse: Mouse) -> void:
	var worth: float = mouse.worth
	kill_count += 1
	kill_worth += mouse.worth
	add_coins(worth)
	update_UI_mousebar()

func _on_mouse_reached_cheese(mouse: Mouse) -> void:
#	kill_count += 1
	stolen_count += 1
	add_life(-mouse.worth)
	update_UI_mousebar()

# connected to cat clicked event by the build cat
func _on_cat_clicked(cat: Cat) -> void:
	print('clicked cat %s' % cat.name)
	if cat == cat_selected:
		unselect_cat(cat)
	else:
		if is_instance_valid(cat_selected):
			cat_selected.deselect()
		select_cat(cat)
		
func unselect_cat(cat: Cat) -> void:
	print('unselecting cat')
	cat_selected = null
	cat.deselect()
	hide_tooltip_on(cat)

func select_cat(cat: Cat) -> void:
	cat.select()
	cat_selected = cat
	show_tooltip_on(cat, 0)

func _on_cat_shoot(bullet: Bullet) -> void:
	# warning-ignore:return_value_discarded
	bullet.connect("hit", Callable(self, "_on_bullet_hit"))

func _on_bullet_hit(bullet: Bullet) -> void:
	var amp: float = 0.1 + bullet.damage/30.0
	if bullet.aoe:
		amp *= 3
	amp = clamp(amp, 0.1, 5)
	var duration: float = 0.08 + 0.03 * amp / 5.0
	$Camera3D.shake(amp, duration)

func _on_SettingsButton_pressed() -> void:
	toggle_pause()

func _on_EllapsedTimer_timeout() -> void:
#	if Global.DEBUG_WIN:
#		Engine.time_scale = 4
#	elif Engine.time_scale > 1:
#		Engine.time_scale = 1
	if kill_count + stolen_count >= spawn_max:
		$EllapsedTimer.stop()
		return
	ellapsed += 1
	update_UI_time()


func _on_NextButton_pressed() -> void:
	pause_game(false)
# warning-ignore:return_value_discarded
	get_tree().change_scene_to_file("res://scenes/maps/%s.tscn" % next_map)


func _on_ButtonSpeed1_pressed() -> void:
	if state != State.READY: return
	print('pressed speed 1')
	var button: Button = $"%ButtonSpeed1"
	var text: String = button.text
	if text == '1':
		Engine.time_scale = 2
		button.text = '2'
		button.icon = preload("res://assets/play2.png")
	elif text == '2':
		Engine.time_scale = 4
		button.text = '4'
		button.icon = preload("res://assets/play4.png")
	elif text == '4':
		Engine.time_scale = 1
		button.text = '1'
		button.icon = preload("res://assets/play.png")
	
func _on_ButtonSpeed2_pressed() -> void:
	if state != State.READY: return
	print('speed 2 pressed')
	Engine.time_scale = 2

func _on_ButtonSpeed4_pressed() -> void:
	if state != State.READY: return
	print('speed 4 pressed')
	Engine.time_scale = 4
